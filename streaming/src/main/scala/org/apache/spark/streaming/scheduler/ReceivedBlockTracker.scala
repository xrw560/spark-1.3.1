...
private[streaming] class ReceivedBlockTracker(
    ...
    // 如果你开启了预写日志机制，这儿还有LogManager，ReceiverTracker接收到数据时，也会判断
    // 如果开启了预写日志机制，那么就写一份儿到预写日志中
    ...
    private def recoverFromWriteAheadLogs(): Unit = synchronized {
        ...
        def insertAllocatedBatch(batchTime: Time, allocatedBlocks: AllocatedBlocks) {
            ...
            streamIdToUnallocatedBlockQueues.values.foreach {
                _.clear()
            }
            lastAllocatedBatchTime = batchTime
