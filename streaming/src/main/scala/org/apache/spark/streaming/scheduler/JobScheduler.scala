...
class JobScheduler(val ssc: StreamingContext) extends Logging {
    ...
    /**
      *  StreamingContext的start()方法，其实是比较薄的一层
      *  真正重要的，是来调用JobScheduler的start()方法
      */
    def start(): Unit = synchronized {
        ...
        // 很重要，创建了ReceiverTracker组件，这是重量级的组件，数据接收相关
        receiverTracker = new ReceiverTracker(ssc)
        // 并启动，我们说的StreamingContext相关的几个重要组件，都创建出来了
        // 然后，就是启动输入DStream关联的Receiver
        // 逻辑都在ReceiverTracker的start()方法中
        receiverTracker.start()

        // 这是JobGenerator，创建JobScheduler的时候，直接就把JobGenerator给创建出来了
        // 这里是启动
        jobGenerator.start()
