...
class ReceiverTracker(ssc: StreamingContext, skipReceiverLaunch: Boolean = false) extends Logging {
    ...
    // 这个receiverInputStreams,就是从StreamingContext的graph中，取出来的
    // DStreamGraph里面取出来的
    ...
    def start() = synchronized {
        ...
        if (!receiverInputStreams.isEmpty) {
            actor = ssc.env.actorSystem.actorOf(Props(new ReceiverTrackerActor),
                "ReceiverTracker")
            // 这个start()方法中，主要是调用了其内部的receiverExecutor的start()方法
    ...
    def cleanupOldBlocksAndBatches(cleanupThreshTime: Time) {
        ...
        if (ssc.conf.getBoolean("spark.streaming.receiver.writeAheadLog.enable", false)) {
            ...
            receiverInfo.values.flatMap { info => Option(info.actor) }
                    .foreach {
                        _ ! CleanupOldBlocks(cleanupThreshTime)
                    }
        }
    }
    ...
    class ReceiverLauncher {
        ...
        /**
          * ReceiverTracker的start()方法，其实启动了内部的一个线程
        ...
        private def startReceivers() {
            ...
            // 只是定义而已，所以，一直到后面，都只是定义，不是在这里执行的，不是在driver执行的
