...
/**
  * 在创建和完成StreamingContext的初始化之后，创建了DStreamingGraph、JobScheduler等关键组件之后
  * 就会调用StreamingContext的socketTextStream等方法，来创建输入DStream
  * 然后针对输入DStream执行一系列的transformation转换操作
  * 最后，会执行一个output输出操作，来触发针对一个一个的batch的job的触发和执行
  *
  * 上述初始化操作完成之后
  * 有一个方法是必须必须要调用的，不调用的话，相当于整个Spark Streaming应用程序不会执行
  * StreamingContext.start()，启动一个Spark Streaming应用程序
  * 这个start()方法，其实最最重要的，就是创建StreamingContext相关的另外两个最最重要的组件：ReceiverTracker、
  * JobGenerator
  * 另外，就是最重要的，启动整个Spark Streaming应用程序的输入DStream，对应的Receiver
  * 不要忘了，是在Spark集群的某个Worker节点上的Executor中启动的
  */
