...
private[spark] class MemoryStore(blockManager: BlockManager, maxMemory: Long)
    ...
    // MemoryStore中维护的entries map，其实就真的是存放的是每个block的数据了！！
    ...
    /**
      ...
      */
    /**
      * 优先放入内存，不行的话，尝试移除部分旧数据，再将block存入
      */
    private def tryToPut(
        ...
        // 这里，也会进行多线程并发同步
        // 这里的多线程并发同步很重要！！！
        // 你要是在这里不进行多线程并发同步的话，那么就不好了！
        // 因为可能你刚判定内存足够你放，但是其他线程就放入了数据！
        // 然后，此时，你往内存中一放数据，直接OOM，内存溢出
        accountingLock.synchronized {
            // 调用ensureFreeSpace方法，判断内存是否够用，如果不够用，此时会将部分数据用dropFromMemory()方法尝试
            // 写入磁盘，但是如果持久化级别不支持磁盘，那么数据丢失
