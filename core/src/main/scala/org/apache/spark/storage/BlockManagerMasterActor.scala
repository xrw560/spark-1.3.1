...
class BlockManagerMasterActor(val isLocal: Boolean, conf: SparkConf, listenerBus: LiveListenerBus)
    ...
    private def register(id: BlockManagerId, maxMemSize: Long, slaveActor: ActorRef) {
        ...
        if (!blockManagerInfo.contains(id)) {
            ...
            // 所以这里要判断一下，如果blockManagerByExecutor map里有BlockManagerId，那么做一下清理
            ...
            // 为blockManagerId创建爱你一份BlockManagerInfo
    ...
    private def updateBlockInfo(
        ...
        // 因为，如果将StorageLevel设置成带着_2的这种，那么就需要将block replicate一份，放到其他BlockManager上
        // blockLocations map ，其实保存了每个blockId对应的BlockManagerId的set集合
        // 所以这里，会更新blockLocations中的信息，因为是用set存储BlockManagerId，因为自动就去重了
...
private[spark] class BlockManagerInfo(
    ...
    def updateBlockInfo(
        ...
        if (_blocks.containsKey(blockId)) {
            ...
            // 判断，如果storageLevel是使用内存，那么就给剩余内存数量加上当前的内存量
            if (originalLevel.useMemory) {
                _remainingMem += originalMemSize
            }
        }

        // 给block创建一份BlockStatus，然后根据其持久化级别，对相应的内存资源进行计算
        if (storageLevel.isValid) {
