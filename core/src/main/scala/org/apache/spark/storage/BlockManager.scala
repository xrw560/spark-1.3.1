...
private[spark] class BlockManager(
    ...
    // 每个BlockManager，会维护一个map
    // 这里维护的blockInfo，可以用于代表一个block
    // 然后，blockInfo最大的作用，就是说，用于作为多线程并发访问同一个block的同步监视器
    ...
    def initialize(appId: String): Unit = {
        // 旧版本中，BlockManager是使用BlockManagerWorker，来针对远程的BlockManager进行数据传输的
        // 但是在spark1.3.0版本中，已经更新为BlockTransferServie
        ...
        // 使用了哪些东西？executeId(每个BlockManager都关联一个executor，BlockTransferService的hostname)、
        ...
        // 使用BlockMnagerActor的引用，进行BlockManager的注册
    ...
    private def doGetLocal(blockId: BlockId, asBlockResult: Boolean): Option[Any] = {
        ...
        if (info != null) {
            ...
            // 所以，BlockInfo，相当于是对一个Block，用于多线程并发访问的同步监视器
            info.synchronized {
                ...
                // 如果其他线程在操作这个block
                // 那么，其实会卡住，等待获取blockInfo的排他锁
                // 如果始终没i有获取到，返回false，直接返回
    ...
    : Seq[(BlockId, BlockStatus)] = {
        ...
        // 重要！！！
        // 如果我们的持久化级别，是定义了_2这种后缀，说明需要对block进行replica，然后传输到其他节点上
            ...
            getRandomPeer() match {
                case Some(peer) =>
                    try {
                        ...
                        // 使用BlockTransferService，将数据异步写入其他的BlockManager上
