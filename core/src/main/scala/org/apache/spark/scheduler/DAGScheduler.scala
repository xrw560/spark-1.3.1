...
/**
  ...
  * 输出是否被物化了(物化是说，写入了磁盘或者内存等地方)，并且寻找一个最小消耗(最优、最少)调度机制来运行Job。
  * 它会将stage作为tasksets提交到底层的TaskSchedulerImpl上，来在集群上运行它们(task)。
  *
  * 除了处理stage的DAG，它还负责决定运行每个task的最佳位置，基于当前的缓存状态，既爱那个这些最佳位置提交给底层的
  * TaskSchedulerImpl。此外，它会处理由于shuffle输出文件丢失导致的失败，在这种情况下，旧的stage可能就会被重新
  * 提交。一个stage内部的失败，如果不是由于shuffle文件丢失所导致的，会被TaskScheduler处理，它会多次重试每一个task，
...
class DAGScheduler(
    ...
    /**
      ...
      * 如果它最后一个rdd的所有依赖，都是窄依赖，ok，那么就不会创建任何新到stage
      * 但是，只要发现这个stage的宽依赖了某个rdd，那么就用宽依赖的那个rdd，创建一个新的stage
      * 然后立即将新的stage返回
      */
    ...
    private[scheduler] def handleJobSubmitted(jobId: Int,
        ...
        try {
            ...
            // 创建一个stage对象
            // 并且将stage加入DAGScheduler内部的内存缓冲中
        ...
        if (finalStage != null) {
            ...
            if (shouldRunLocally) {
                ...
                // 并且导致其他的stage，都放入waitingStages队列里
                submitStage(finalStage)

                // stage划分算法，实在是太重要了，因为对于Spark高手，或者Spark精通人员来说
                // 必须对stage划分算法很清晰，知道你自己编写的Spark Application被划分了几个job
                // 每个job被划分了几个stage
                // 每个stage，包括了你的哪些代码
                // 只有知道了每个stage包括了你的哪些代码之后
    ...
    /**
      * 提交stage的方法
      * 这个，其实就是stage划分算法的入口
      * 但是，stage划分算法那，其实是由submitStage()方法和getMissingParentStages()方法共同组成的
      */
    private def submitStage(stage: Stage) {
        ...
        if (jobId.isDefined) {
            ...
            if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
                //TODO 获取他的父Stage
                // 调用getMissingParentStages方法，去获取当前这个stage的父stage
                ...
                if (missing == Nil) {
                    ...
                    //并且将当前stage，放入waitingStage等待执行的stage队列中
                    waitingStages += stage
                }
            }
        } else {
    ...
    private def submitMissingTasks(stage: Stage, jobId: Int) {
        ...
        if (tasks.size > 0) {
            ...
            // 默认情况下，我们的standalone模式，是使用TaskSchedulerImpl,askScheduler只是一个trait
    ...
    : Seq[TaskLocation] = {
        ...
        // 最后，递归调用自己，去寻找rdd的父rdd，看看对应的partition是否缓存或者checkpoint
        ...
        // 如果这个stage，从最后一个rdd，到最开始的rdd，partition都没有被缓存或者checkpiont
        // 那么，task的最佳位置(preferredLocs)，就是Nil，没有最佳位置，它的位置就要通过后面的taskScheduler去决定怎么给它分配
        Nil
    }
