...
class SQLContext(@transient val sparkContext: SparkContext)
    ...
    /**
      ...
      */
    /**
      * 如果要给Spark SQL设置一些参数，那么要使用SQLContext.setConf()方法
      * SQLContext.setConf()方法，底层是会将配置信息放入SQLConf对象中的
      */
    ...
    /**
      ...
      */
    /**
      * 使用Spark执行一条SQL查询语句，将结果作为DataFrame返回，SQL解析使用的方言，可以通过spark.sql.dialect
      * 参数，来进行设置
      */
    def sql(sqlText: String): DataFrame = {
        // 首先，第一步，就会去查看我们通过SQLContext.setConf()方法设置的参数
        // spark.sql.dialect，如果是sql方言，那么就进入接下来的执行
        // 这里，可以通过源码清晰地看到，如果方言不是sql，那么就直接报错
        // conf，就是SQLConf
        if (conf.dialect == "sql") {
