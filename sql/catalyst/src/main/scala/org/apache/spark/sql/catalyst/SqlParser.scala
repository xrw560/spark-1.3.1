...
class SqlParser extends AbstractSparkSQLParser with DataTypeParser {
    ...
    /**
      * 从这里看出来，Spark SQL是支持至少两种主要的SQL语法的，包括select语句和insert语句
      */
    ...
    /**
      * 这里，就是对select语句执行解析，select里面，包括from、where、group、having、sort、limit
      */
    ...
    /**
      * 会将你的SQL语句里面解析出来的各种token，或者TreeNode
      * 给关联起来，最后组成一颗语法树
      * 语法树即封装在LogicalPlan中，但是要注意，此时的LogicalPlan，还是Unresolved LogicalPlan
      */
    ...
    protected lazy val function: Parser[Expression] =
        (SUM ~> "(" ~> expression <~ ")" ^^ { case exp => Sum(exp) }
                ...
                | APPROXIMATE ~> "(" ~> floatLit ~ ")" ~ COUNT ~ "(" ~ DISTINCT ~ expression <~ ")" ^^ { case s ~ _ ~ _ ~ _ ~ _ ~ e => ApproxCountDistinct(e, s.toDouble) }
                ...
                | IF ~ "(" ~> expression ~ ("," ~> expression) ~ ("," ~> expression) <~ ")" ^^ { case c ~ t ~ f => If(c, t, f) }
            ...
            case casePart ~ altPart ~ elsePart =>
                ...
                | ident ~ ("(" ~> repsep(expression, ",")) <~ ")" ^^ { case udfName ~ exprs => UnresolvedFunction(udfName, exprs) }
                )
    ...
    protected lazy val primary: PackratParser[Expression] =
        (literal
                | expression ~ ("[" ~> expression <~ "]") ^^ { case base ~ ordinal => GetItem(base, ordinal) }
                | (expression <~ ".") ~ ident ^^ { case base ~ fieldName => UnresolvedGetField(base, fieldName) }
                | cast
                | "(" ~> expression <~ ")"
                | function
                | dotExpressionHeader
