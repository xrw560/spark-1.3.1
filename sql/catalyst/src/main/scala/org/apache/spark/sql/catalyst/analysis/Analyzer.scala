...
/**
  ...
  */
/**
  * Analyser的父类是RuleExecutor
  * 所以呢，其实，调用Analyser的apply()方法时，实际上会调用到RuleExecutor的apply()方法，并
  * 传入一个Unresolved LogicalPlan
  */
class Analyzer(
        catalog: Catalog,
            ...
            case q: LogicalPlan =>
                ...
                q transformExpressionsUp {
                    ...
                    case u@UnresolvedAttribute(name) =>
                        ...
                        val result =
                            withPosition(u) {
                                q.resolveChildren(name, resolver).getOrElse(u)
                            }
